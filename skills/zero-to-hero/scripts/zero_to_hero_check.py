...
REQUIRED = [
    ...
    'prompts/02-canonical-docs-pack.md','prompts/03-design-visual-pack.md',
    'prompts/04-hardware-mechanical-pcb-pack.md','prompts/05-frontend-parity-system.md',
    'prompts/06-product-usability-contract.md','prompts/07-local-product-done-harness.md',
    'prompts/08-omx-handoff.md','prompts/09-canonical-cleanup.md',
...
def prompt_inventory(skill: Path) -> dict:
    ...
    expected = [f'{i:02d}-' for i in range(0, 11)]
    missing_expected = [prefix for prefix in expected if not any(name.startswith(prefix) for name in names)]
    return {'check': 'prompt_sequence', 'ok': not dupes and not missing_expected, 'prompt_count': len(names), 'duplicates': dupes, 'missing_expected': missing_expected}
...
def metadata(skill: Path) -> dict:
    ...
    if manifest.exists():
        try:
            ...
            ok = False; details['manifest_error'] = str(exc)
...
def reference_smoke(skill: Path) -> dict:
    ...
    for p in roots:
        ...
        for m in link_re.finditer(p.read_text(errors='ignore')):
            ...
            if target.endswith(('.png','.jpg','.jpeg','.svg','.gif','.webp')): continue
            candidate=(p.parent/target).resolve()
            try: candidate.relative_to(skill)
            except Exception: continue
            if not candidate.exists(): failures.append({'path': str(p.relative_to(skill)), 'target': target})
...
def main() -> int:
    ...
    args=ap.parse_args()
    if args.list_checks:
        print(json.dumps(list_checks(), indent=2)); return 0
    skill=resolve_skill(args.path); start=time.time(); checks=[]
    planned=[]
    planned.extend(exists_checks(skill, REQUIRED))
    planned.extend([prompt_inventory(skill), metadata(skill), no_runtime_artifacts(skill)])
    if args.deep:
